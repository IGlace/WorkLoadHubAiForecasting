...
- A **member** is a person counted in the workload, identified by a UUID string (`id`) and named by `name`. Weeks start on Monday; working days are Monday to Friday; public holidays are off days. A **forecast window** is five weekdays starting the first weekday after the run day (a run on a Friday or a weekend starts on Monday); a run covers two contiguous windows; a holiday inside a window stays inside with zero capacity and no predicted arrivals. Weeks remain the unit of history.
...
- The row identifies its window by `window` (1 or 2), `start` and `end` (ISO dates).
