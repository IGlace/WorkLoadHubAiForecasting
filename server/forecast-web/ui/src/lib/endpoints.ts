...
export const ENDPOINTS: Endpoint[] = [
  ...
  { method: 'GET', path: '/api/directory/users', group: 'Directory', who: 'any acting user', production: true,
    purpose: 'Every user with their effective role — the one their job title gives them, not users.role — and their place: the team they lead, the one they belong to, and, for a skill team leader, the teams beneath them. The module answers in ids; the host joins the names.',
  ...
  { method: 'GET', path: '/api/directory/teams', group: 'Directory', who: 'any acting user', production: true,
    purpose: 'Every team: a team leader and the people who report to them directly. The id is the leader\'s own user id, and reportsTo is the manager that leader answers to — usually a skill team leader, who leads no team themselves.',
    response: '[{id, name, managerId, managerName, reportsToId, reportsToName, memberCount}]', errors: ['401'] },
  { method: 'GET', path: '/api/directory/teams/{id}', group: 'Directory', who: 'any acting user', production: true,
    purpose: 'One team with its members: its leader, and every counted person reporting to them.',
    response: '{id, name, managerId, managerName, reportsToId, reportsToName, members: [{id, fullName, role, jobTitle}]}', errors: ['401', '404 TEAM_NOT_FOUND'] },
  ...
  { method: 'GET', path: '/api/permissions', group: 'Permissions', who: 'any acting user (a test tool for the owner)', production: false,
    purpose: 'What any user may do on any team, with the reasons: the role rules of design 2026-09-11, section 3.2, rebound to the hierarchy and the job-title role by design 2026-09-21.',
