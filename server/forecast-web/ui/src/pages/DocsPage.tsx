...
export function DocsPage() {
  ...
  return (
    <div className="docs">
      ...
      <div className="card">
        <p><b>The module</b>, <code>forecast-core</code>, is one Maven dependency. Its auto-configuration sees the host's own <code>DataSource</code>, runs its Flyway migrations under its own history table (<code>forecast_schema_history</code>) and registers <code>ForecastService</code>, <code>GitHubTokenStore</code> and everything under them. Nine service methods are the whole interface: <code>startRun</code>, <code>progress</code>, <code>getRun</code>, <code>listRuns</code>, <code>currentForecast</code>, <code>accuracy</code>, <code>narrate</code>, <code>narrative</code>, <code>copilotStatus</code>; the token store adds <code>save</code>, <code>has</code>, <code>clear</code>.</p>
        <p><b>The host</b> (this application's <code>host</code> package) adds what the module deliberately leaves out: the role check before every call (<code>ForecastAccess</code>), the run registry <code>(runId, teamId, requestedBy)</code> in its own table so a restart can still authorise a poll, one run at a time for a skill team leader, the narration executor (two daemon threads, one narration per run and language in flight), and the HTTP translation (<code>HostForecastFacade</code>, the controllers, <code>ApiExceptionHandler</code>). The production server writes these classes against its session instead of the <code>X-Acting-User</code> header; nothing else differs.</p>
        <p><b>Demo only</b> (the <code>demo</code> package, deleted by production): the seeded SQLite file, the demo clock, the generated token key file, the served front end, and the routes marked <span className="badge">demo</span> below.</p>
        <p className="small muted">Reference: <code>server/README.md</code> ("Using the module from the WorkloadHub server"), <code>docs/superpowers/specs/2026-09-11-host-integration-design.md</code>, <code>docs/superpowers/specs/2026-09-18-forecast-web-and-showcase-ui-design.md</code>.</p>
      </div>

      <h2>2. The flow of a run</h2>
      <div className="card">
        <ol>
          <li>The page asks to forecast a team: <code>POST /api/teams/{'{teamId}'}/forecast-runs</code>. The host checks the role, calls <code>startRun(new RunRequest(teamId, userId))</code>, records the run in its registry and answers <b>202</b> with the id. The run computes on the module's own pool (<code>whf.run-threads</code>).</li>
      ...
      <div className="card scroll">
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>forecast-web.*</code></td><td colSpan={2}>Demo wiring of this host only: the SQLite file and its seed, the demo clock, the token key file, the served UI directory. Production has none of them.</td></tr>
          </tbody>
        </table>
      </div>
